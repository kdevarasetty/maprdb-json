...
@AboutTest({ @Property(name = "test.name", value = "CRUD"),
		...
		@Property(name = "data.file", value = "crud_user.json") })
public class CRUD extends TestConfig {
	...
	@OverrideConf({ @Property(name = "bean.size", value = "5"),
			...
			@Property(name = "data.file", value = "crud_samplebean_insertseveralbeans.json"),
	...
	@OverrideConf({
			@Property(name = "data.file", value = "crud_samplebean_insertseveralbeansWithoutAddress.json"),
			@Property(name = "test.category", value = "insert several documents"),
			@Property(name = "test.keywords", value = "documents, several, multiple, insert, all") })
	...
	@OverrideConf({
			@Property(name = "depends", value = "createDocuments"),
	...
	/**
	 * Reference method to see the whole code in one place. Update record see
	 * how to : update existing field can add/remove attribute to a document,
	 * append data into list of values. This method has been split into multiple
	 * methods as it has multiple test cases.
	 */
	@OverrideConf({ @Property(name = "depends", value = "createDocuments"),
			@Property(name = "test.category", value = "update"),
			@Property(name = "test.keywords", value = "update, reference"),
			@Property(name = "test.name", value = "update document with known values") })
	void updateDocuments() throws IOException {

		{
			out.println("\t\tAdd address and status to jdoe");
			out.println("before :\t" + table.findById("jdoe"));

			// create a mutation
			DocumentMutation mutation = MapRDB.newMutation().set("active", true)
					.set("address.city", "Redwood City")
					.set("address.pincode", "94065");

			table.update("jdoe", mutation);
			table.flush();

			out.println("after :\t\t" + table.findById("jdoe"));
		}

		{
			...
			// create a mutation
			DocumentMutation mutation = MapRDB.newMutation().append("interests",
					Collections.singletonList("development"));
			...
			out.println("after :\t\t" + table.findById("mdupont",
					"first_name", "last_name", "interests"));
		}
	...
	/**
	 * Reference method. Query the record. This method also has been split into
	 * multiple methods as it has multiple test cases.
	 */
	@OverrideConf({ @Property(name = "depends", value = "createDocuments"),
			@Property(name = "test.category", value = "query"),
			@Property(name = "test.keywords", value = "query, reference"),
			@Property(name = "test.name", value = "query table with known values") })
	private void queryDocuments() throws Exception {

		{
			...
			out.println("Id : " + record.getIdString()
					+ " - first name : " + record.getString("first_name"));
		}

		{
			// get a single document
			Document record = table.findById("mdupont", "last_name");
			out.print("Single record with projection\n\t");
			out.println(record);
...

		{
			...
			out.println("User Pojo from document : " + user.toString());
		}
...

		{
			...
			try (DocumentStream documentStream = table.find("first_name",
					"last_name")) {
...

		{
			...
			// it is interesting to see how you can ignore unknown attributes
			// with the JSON Annotations
			out.println("\n\nAll records with a POJO");

			try (DocumentStream documentStream = table.find()) {
				for (Document doc : documentStream) {
					out.println("\t" + doc.toJavaBean(User.class));
				}
			}
...

		{
			// Condition in sub document
			QueryCondition condition = MapRDB.newCondition()
					.is("address.zip", EQUAL, 95109).build();
			out.println("\n\nCondition: " + condition);
			try (DocumentStream documentStream = table.find(condition)) {
				for (Document doc : documentStream) {
					out.println("\t" + doc);
	...
	@OverrideConf({
			...
			@Property(name = "use.data.file", value = "crud_samplebean_insertseveralbeans.json") })
